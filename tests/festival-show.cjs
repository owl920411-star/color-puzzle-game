...
(async()=>{
 ...
 check('fever gets a separate celebration',await page.evaluate(()=>document.querySelector('.festival-banner').textContent.includes('FEVER PARTY')));
 check('particle count stays bounded under repeated celebrations',await page.evaluate(()=>{for(let i=0;i<100;i++)FestivalShow.fever();return FestivalShow.inspect().particles<=FestivalShow.inspect().cap;}));
 ...
 await page.evaluate(()=>{total=T.totalGoal;stageClear();});check('clear triggers finale',await page.evaluate(()=>FestivalShow.inspect().finale));await page.clock.runFor(1000);
 check('next stage removes all show effects and animation work',await page.evaluate(()=>si===4&&!FestivalShow.inspect().active&&!FestivalShow.inspect().scheduled&&FestivalShow.inspect().particles===0&&!document.querySelector('.festival-banner')));
