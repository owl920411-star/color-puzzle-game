...
function findRun(){return saved.runs.find(r=>r.mode===mode&&r.seed===game.seed);}
...
function updateHUD(){
 ...
 $('mode-label').textContent=modes[mode];$('seed-label').textContent=mode==='tutorial'?'LEARN':state==='menu'?'READY':game.seed.slice(-9);
...
function finish(reason){if(state==='end')return;state='end';endingReason=reason;resetInput();game.active=null;phase=null;screen.hidden=false;screen.className='screen small';if(mode==='tutorial'){saved.learned=true;save();content.innerHTML='<div class="screen-kicker">FIRST RESONANCE</div><h2>균열이 이어졌어요.</h2><p>완성한 줄에서 충격이 퍼져<br>연결된 유리까지 함께 깨집니다.<br>다음에는 직접 길을 만들어보세요.</p><button class="primary" data-screen="start-sprint">3분 도전 시작</button><button class="secondary" data-screen="tutorial">한 번 더 연습</button>';setStatus('균열이 닿지 않은 조각은 남고, 아래로 내려옵니다.');}
  ...
  if(!gameResultSaved){
   saved.runs=saved.runs.filter(r=>r.mode!==mode||r.seed!==game.seed);
   saved.runs.push({mode,seed:game.seed,attempts,best:sameBest,last:run});saved.runs=saved.runs.slice(-12);
   ...
   saved.best[bestKey()]=Math.max(prev,game.score);if(game.extra>0)saved.badges.fracture=true;if(game.maxChain>=2)saved.badges.cascade=true;if(game.shards>=100)saved.badges.glass100=true;saved.plays=(Number(saved.plays)||0)+1;save();gameResultSaved=true;
  }
  const rows=[['lines','제거한 줄','줄'],['extra','추가 파쇄','개'],['efficiency','조각당 제거 유리','개']].map(([key,label,unit])=>`<tr><th scope="row">${label}</th><td>${run[key]}${unit}</td><td>${Number.isFinite(previous?.[key])?signed(Math.round((run[key]-previous[key])*100)/100)+unit:'—'}</td></tr>`).join('');
  content.innerHTML=`<div class="screen-kicker">${game.score>prev?'NEW PERSONAL BEST':'YOUR RESONANCE'}</div><h2>${reason==='time'?'빛나는 3분.':'다음 균열을 향해.'}</h2><div class="result-score">${game.score.toLocaleString()}</div><div class="run-reflection">${reflection}<br>같은 판 최고 ${sameBest.toLocaleString()}점 · ${attempts}회 도전</div><table class="replay-stats"><caption>직전 동일 조건과 비교</caption><thead><tr><th>기록</th><th>이번</th><th>직전 대비</th></tr></thead><tbody>${rows}</tbody></table><p class="replay-note">조각당 제거 유리 = 제거한 유리 ÷ 배치한 조각<br>이 모드 최고 연쇄 ${saved.records[bestKey()].chain} · 균열 ${saved.records[bestKey()].extra}</p><p>${reason==='time'?'시간이 끝났어요. 같은 조각으로<br>더 큰 연쇄를 만들어볼까요?':'유리가 입구까지 차올랐어요.<br>다른 배치로 다시 도전해보세요.'}</p><button class="primary" data-screen="retry">같은 판 최고 기록에 도전</button><button class="secondary" data-screen="new">${mode==='daily'?'오늘의 도전 다시':'새로운 판 시작'}</button><button class="text-btn" data-screen="menu">모드 선택으로</button>`;
