...
let repeat=null,drag=null,keyHeld=new Set(),soundOn=!!saved.sound,audio=null,renderRatio=1;
...
function resetInput(){repeat=null;drag=null;keyHeld.clear();$('touchpad').classList.remove('engaged');$('pad-hint').textContent='손가락을 편한 곳에 올려보세요';for(const b of document.querySelectorAll('[data-action]'))b.classList.remove('pressed');}
...
function action(a){if(state!=='playing'||!game.active)return false;audioInit();let moved=false;const grounded=!game.fits(game.active,0,1);if(a==='left'||a==='right')moved=game.move(a==='left'?-1:1);else if(a==='rotate'){moved=game.rotate();if(moved)tone('rotate');}else if(a==='down'){moved=game.move(0,1);if(moved){game.score++;fallTime=0;}}else if(a==='drop'){const d=game.dropDistance();dropTrail={cells:game.active.cells.map(c=>({...c,x:c.x+game.active.x,y:c.y+game.active.y})),distance:d,life:170};game.move(0,d);game.score+=d*2;tone('drop');lock();updateHUD();return true;}else if(a==='hold'&&mode!=='tutorial'){moved=game.hold();if(game.over){finish('top');return false;}if(moved){resetInput();lockTime=0;lockResets=0;fallTime=0;renderPreviews();}}
 if(moved&&grounded&&lockResets<12&&(a==='left'||a==='right'||a==='rotate')){lockTime=0;lockResets++;}updateHUD();return moved;}
...
function glass(c,x,y,size=36,options={}){const g=options.context||ctx,[light,dark]=colors[c.type]||colors.I;g.save();if(options.alpha!==undefined)g.globalAlpha=options.alpha;const m=Math.max(1.4,size*.055),r=Math.max(2,size*.08),px=x+m,py=y+m,s=size-m*2;
 if(options.ghost){g.strokeStyle=light;g.globalAlpha=.45;g.setLineDash([3,3]);g.lineWidth=1.1;g.strokeRect(px+.5,py+.5,s-1,s-1);g.restore();return;}
...
// A short tap rotates. A drag commits to one axis and never hard-drops.
for(const surface of [canvas,$('touchpad')]){
 surface.addEventListener('pointerdown',e=>{
  if(state!=='playing'||!game.active||drag||e.button!==0)return;
  e.preventDefault();surface.setPointerCapture?.(e.pointerId);repeat=null;
  drag={id:e.pointerId,surface,pieceId:game.active.cells[0].id,startX:e.clientX,startY:e.clientY,lastX:e.clientX,lastY:e.clientY,started:performance.now(),axis:null,travel:0,carry:0,direction:0,stepped:false,unit:Math.max(20,Math.min(32,canvas.getBoundingClientRect().width/10))};
  $('touchpad').classList.add('engaged');$('pad-hint').textContent='좌우로 밀기 · 탭 회전 · 아래로 천천히';
 });
 surface.addEventListener('pointermove',e=>{
  const d=drag;if(!d||d.id!==e.pointerId||d.surface!==surface||state!=='playing'||game.active?.cells[0].id!==d.pieceId)return;
  e.preventDefault();
  const totalX=e.clientX-d.startX,totalY=e.clientY-d.startY;
  const deltaX=e.clientX-d.lastX,deltaY=e.clientY-d.lastY;
  d.lastX=e.clientX;d.lastY=e.clientY;
  d.travel=Math.max(d.travel,Math.hypot(totalX,totalY));
  if(!d.axis){
   if(Math.max(Math.abs(totalX),Math.abs(totalY))<8)return;
   d.axis=Math.abs(totalX)>=Math.abs(totalY)?'x':'y';
   d.carry=d.axis==='x'?totalX:totalY;
  }else d.carry+=d.axis==='x'?deltaX:deltaY;
  const direction=Math.sign(d.carry);
  if(direction&&direction!==d.direction){d.stepped=false;d.direction=direction;}
  let count=0;
  while(drag===d&&Math.abs(d.carry)>=(d.stepped?d.unit:8)&&count++<10){
   const step=Math.sign(d.carry),threshold=d.stepped?d.unit:8;
   const moved=d.axis==='x'?action(step>0?'right':'left'):step>0?action('down'):false;
   d.carry-=step*threshold;d.stepped=true;
   if(!moved){d.carry=0;break;}
  }
  $('pad-hint').textContent=d.axis==='x'?'좌우 이동 중':'천천히 내리기 · 낙하는 버튼으로';
 });
 const release=(e,cancelled=false)=>{
  const d=drag;if(!d||d.id!==e.pointerId||d.surface!==surface)return;
  drag=null;$('touchpad').classList.remove('engaged');$('pad-hint').textContent='손가락을 편한 곳에 올려보세요';
  if(!cancelled&&!d.axis&&d.travel<8&&Math.hypot(e.clientX-d.startX,e.clientY-d.startY)<8&&performance.now()-d.started<280&&game.active?.cells[0].id===d.pieceId)action('rotate');
 };
 surface.addEventListener('pointerup',e=>release(e));
 surface.addEventListener('pointercancel',e=>release(e,true));
 surface.addEventListener('lostpointercapture',e=>release(e,true));
}
$('touchpad').addEventListener('click',e=>{if(e.detail===0)action('rotate');});
const keys={ArrowLeft:'left',ArrowRight:'right',ArrowDown:'down',ArrowUp:'rotate',x:'rotate',X:'rotate',' ':'drop',c:'hold',C:'hold'};document.addEventListener('keydown',e=>{if($('help-dialog').open)return;if(e.key==='Escape'||e.key==='p'||e.key==='P'){if(e.repeat)return;e.preventDefault();if(state==='paused')resume();else pause();return;}if(e.key===' '&&e.target.closest?.('button,a,input,select,textarea,[contenteditable]'))return;const a=keys[e.key];if(!a||state!=='playing')return;e.preventDefault();if(e.repeat||keyHeld.has(e.key))return;keyHeld.add(e.key);action(a);if(['left','right','down'].includes(a)&&state==='playing')repeat={id:e.key,action:a,time:200};});document.addEventListener('keyup',e=>{keyHeld.delete(e.key);if(repeat?.id===e.key)repeat=null;});
