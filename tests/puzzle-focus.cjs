...
(async()=>{
 ...
 const checks=[],errors=[];const check=(name,result)=>{assert.equal(result,true,name);checks.push(name);};
 const legacyKey='flower-bloom.garden.v1',key='farm-friends.puzzle-progress.v1';
 try{
  const page=await browser.newPage({viewport:{width:320,height:568}});page.on('pageerror',e=>errors.push(e.message));
