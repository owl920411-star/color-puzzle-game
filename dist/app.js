...
const materialColors={sand:[['#ffe4a5','#b77630'],['#ffa97a','#b44734'],['#f5f0c2','#aca86a'],['#d9b3f2','#8f67ac']],water:[['#78dfff','#237bad'],['#91ffdc','#218f87'],['#b3aaff','#6556bf'],['#ffafe1','#ab528c']],jelly:[['#ff94c5','#b23875'],['#b6f585','#579744'],['#ffdb77','#b47727'],['#c6a2ff','#7650b0']]};
...
function bestKey(){return(material==='glass'?'':material+':')+(mode==='daily'?'daily-'+(state!=='menu'&&game.seed.startsWith('DAILY-')?game.seed.slice(6):day()):mode);}
...
function findRun(){return saved.runs.find(r=>r.mode===mode&&r.seed===game.seed&&(r.material||'glass')===material);}
...
function finish(reason){if(state==='end')return;state='end';endingReason=reason;resetInput();game.active=null;phase=null;screen.hidden=false;screen.className='screen small';if(mode==='tutorial'){saved.learned=true;save();content.innerHTML='<div class="screen-kicker">FIRST RESONANCE</div><h2>균열이 이어졌어요.</h2><p>완성한 줄에서 충격이 퍼져<br>연결된 유리까지 함께 깨집니다.<br>다음에는 직접 길을 만들어보세요.</p><button class="primary" data-screen="start-sprint">3분 도전 시작</button><button class="secondary" data-screen="tutorial">한 번 더 연습</button>';setStatus('균열이 닿지 않은 조각은 남고, 아래로 내려옵니다.');}
  ...
  if(!gameResultSaved){
   saved.runs=saved.runs.filter(r=>r.mode!==mode||r.seed!==game.seed||(r.material||'glass')!==material);
   saved.runs.push({mode,material,seed:game.seed,attempts,best:sameBest,last:run});saved.runs=saved.runs.slice(-12);
...
function lock(){if(!game.active)return;game.lock();resetInput();chain=0;settleCount=0;if(game.material!=='glass'){beginSettle();return;}const p=E.clearPlan(game.board);if(p)beginClear(p);else afterPiece();}
...
function update(rawDt){const dt=Math.min(100,rawDt);if(state==='playing'||state==='resolving'){elapsed+=rawDt;if(mode==='sprint'||mode==='daily')remaining=Math.max(0,remaining-rawDt);if(state==='playing'&&remaining<=0&&(mode==='sprint'||mode==='daily')){finish('time');return;}if(repeat&&state==='playing'){repeat.time-=dt;let count=0;while(repeat&&repeat.time<=0&&count++<4){const r=repeat;action(r.action);if(repeat===r)r.time+=70;}}if(state==='playing'&&mode!=='tutorial'){if(game.active&&!game.fits(game.active,0,1)){fallTime=0;lockTime+=dt;if(lockTime>=650)lock();}else{fallTime+=dt;lockTime=0;while(state==='playing'&&fallTime>=game.gravity){fallTime-=game.gravity;game.move(0,1);}}}
 if(state==='resolving'){phaseTime+=dt;if(phase==='settle'&&phaseTime>=(reduced?16:42)){phaseTime=0;falls=E.settleStep(game.board,game.material,game.pieces+settleCount++);if(!falls.length){const next=E.materialPlan(game.board,game.material);if(next)beginClear(next);else afterPiece();}}else if(phase==='crack'&&phaseTime>=(reduced?90:260+Math.min(180,Math.max(...plan.cells.map(c=>c.depth))*24))){emit(plan.cells);const result=game.resolve(plan,chain);falls=result.falls;plan=null;if(game.material==='glass'){phase='fall';phaseTime=0;}else beginSettle();updateHUD();}else if(phase==='fall'&&phaseTime>=(reduced?60:240)){falls=[];const p=E.clearPlan(game.board);if(p)beginClear(p);else afterPiece();}}}
