...
function showMenu(){
 showPanel(`<div class="bloom-menu"><div class="bloom-sky-doodles" aria-hidden="true"><i>☁</i><i>♡</i><i>✿</i><i>☆</i></div><div class="bloom-brand"><span class="bloom-flower">✿</span><div><div class="kicker">CRAYON BLOOM</div><h1>크레용블룸</h1><p class="bloom-tagline">작은 블록이 피워내는 색색의 즐거움</p></div><span class="bloom-rainbow">⌒</span></div><div class="mascot-strip" aria-label="크레용블룸 마스코트"><div class="baby-doodle"><span class="curl">〰</span><span class="face">•ᴗ•</span><small>오늘도 같이 놀아요!</small></div><div class="chick-doodle"><span>●</span><b>˃ᴗ˂</b><small>삐약!</small></div></div><div class="modes"><button class="mode-card ${kind==='normal'?'selected':''}" data-menu="normal" aria-pressed="${kind==='normal'}"><canvas class="mode-art" data-art="glass" width="320" height="180"></canvas><strong>꽃길 블록</strong><small>CLASSIC BLOOM</small></button><button class="mode-card ${kind==='sand'?'selected':''}" data-menu="sand" aria-pressed="${kind==='sand'}"><canvas class="mode-art" data-art="sand" width="320" height="180"></canvas><strong>모래 놀이터</strong><small>SAND PLAY</small></button></div><p class="mode-description">${kind==='normal'?'알록달록 블록을 차곡차곡 쌓아 가로줄을 피워보세요.<br>특별한 블록 친구들이 한 판을 더 재미있게 만들어요.':'사르르 흐르는 모래주머니를 모아 연쇄를 만들어보세요.<br>무너질수록 더 신나는 모래 놀이터!'}</p><button class="primary bloom-start" data-menu="start">✿ ${name()} 시작하기</button><div class="menu-best">오늘의 최고 기록 · ${recordBest.toLocaleString()}점</div><button class="text-button" data-menu="settings">놀이 방법 · 설정</button><p class="storage-note">기록은 이 브라우저에 소중히 저장해요 ♡</p></div>`,'menu');
