...
saved.campaign=saved.campaign&&typeof saved.campaign==='object'&&!Array.isArray(saved.campaign)?saved.campaign:{};
...
function addFeedback(clear,gain){
 ...
 feedback.push({x,y,gain,life:1000,total:1000,color:cellColors(clear.cells[0].cell)[0],kind:game.material,rows:clear.rows.slice(),power:Math.min(3,chain),rays:Math.min(18,8+chain*3)});feedback=feedback.slice(-6);vibrate(chain>1?[12,35,12]:12);
}
...
function updateHUD(){
 ...
 if(mode==='stage'){$('stage-hud').textContent='제거 '+Math.min(game.shards,stageConfig?.target||0)+' / '+(stageConfig?.target||0)+'칸';}
...
function start(selected=mode,seed){resetInput();mode=selected;if(mode==='stage'){stageConfig=S.config(stageNumber,material);seed=stageConfig.seed;}game=selected==='tutorial'?E.tutorial():new E.Game(seed||(selected==='daily'?'DAILY-'+day():newSeed()),selected,material,mode==='stage'?'standard':difficulty);if(mode==='stage')S.prepare(game,stageConfig);state='playing';goals=mode==='tutorial'?[]:mode==='stage'?[{key:'shards',label:'블록 제거',target:stageConfig.target,unit:'칸',xp:40+stageNumber}]:P.missions(material,difficulty);goalDone=[];goalToast=0;feedback=[];impact=null;strategy=P.strategy(game.board,game.material);remaining=180000;elapsed=0;fallTime=0;lockTime=0;lockResets=0;chain=0;phase=null;plan=null;falls=[];particles=[];dropTrail=null;calloutTime=0;gameResultSaved=false;endingReason='';settleCount=0;last=performance.now();screen.hidden=true;replayBest=mode==='tutorial'?null:findRun()?.best??legacyRun()?.score??null;$('callout').classList.remove('show');audioInit();setStatus(selected==='tutorial'?'오른쪽에 맞춰져 있어요. 아래로 빠르게 쓸고 떼어보세요.':'톡 터치하면 회전 · 좌우 이동 · 아래로 쓸고 떼면 낙하');updateHUD();renderPreviews();}
function pause(){
...
function emit(cells){
 const kind=game.material,count=reduced?1:kind==='sand'?9:kind==='water'?7:8,cap=reduced?45:260;
