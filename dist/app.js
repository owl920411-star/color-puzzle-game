(() => {
'use strict';
const E=window.GlassEngine,P=window.GlassProgress,$=id=>document.getElementById(id),canvas=$('board'),ctx=canvas.getContext('2d');
...
const materials={glass:{name:'유리',tag:'균열을 잇는 설계',rule:'가로줄을 채우면 연결된 흰 균열까지 파쇄됩니다.',effect:'파쇄'},sand:{name:'모래',tag:'흩어지는 색의 길',rule:'같은 색 모래를 이어 왼쪽 벽과 오른쪽 벽을 연결하세요.',effect:'모래길'},water:{name:'물',tag:'빈틈을 찾아 흐르는 물',rule:'같은 색 물 10칸 이상을 상하좌우로 모으면 배수됩니다.',effect:'배수'},jelly:{name:'젤리',tag:'붙을수록 커지는 탄성',rule:'같은 색 젤리 8칸 이상이 상하좌우로 붙으면 터집니다.',effect:'팝'}};
const materialColors={sand:[['#ffe4a5','#b77630'],['#d9b3f2','#8f67ac']],water:[['#78dfff','#237bad'],['#91ffdc','#218f87'],['#b3aaff','#6556bf'],['#ffafe1','#ab528c']],jelly:[['#ff94c5','#b23875'],['#b6f585','#579744'],['#ffdb77','#b47727'],['#c6a2ff','#7650b0']]};
...
saved.runs=Array.isArray(saved.runs)?saved.runs.filter(r=>r&&['sprint','endless','daily'].includes(r.mode)&&typeof r.seed==='string'&&r.last&&Number.isFinite(r.best)).slice(-12):[];
...
const modes={sprint:'3분 도전',daily:'오늘의 도전',endless:'무한 모드',tutorial:'조작 연습'};
const screen=$('screen'),content=screen.querySelector('.screen-content');
function bestKey(){return(difficulty==='standard'?'':difficulty+':')+(material==='glass'?'':material+':')+(mode==='daily'?'daily-'+(state!=='menu'&&game.seed.startsWith('DAILY-')?game.seed.slice(6):day()):mode);}
...
function addFeedback(clear,gain){
 ...
 feedback.push({x,y,gain,life:850,total:850,color:cellColors(clear.cells[0].cell)[0],kind:game.material});feedback=feedback.slice(-6);vibrate(chain>1?[12,35,12]:12);
}
...
function findRun(){return saved.runs.find(r=>r.mode===mode&&r.seed===game.seed&&(r.material||'glass')===material&&(r.difficulty||'standard')===difficulty);}
...
function updateGoals(){
 ...
 $('goal-title').textContent=goalToast>0?goalToastText:next?`이번 판 ${result.count}/3 · ${next.label}`:'목표 3개 완료 · 보너스 +25 XP';
...
function showMenu(){screen.hidden=false;screen.className='screen lobby';content.innerHTML=`<div class="lobby-heading"><div class="screen-kicker">THE ART OF BREAKING</div><h2>GLASSFALL</h2><p>떨어뜨리고, 터뜨리는 새로운 테트리스</p></div><div class="material-options" role="group" aria-label="소재 선택">${Object.entries(materials).map(([key,m])=>`<button data-material="${key}" class="${material===key?'selected':''}" aria-pressed="${material===key}"><canvas class="mode-art" data-art="${key}" width="320" height="260" aria-hidden="true"></canvas><span class="mode-name">${m.name} 모드</span><small>${key.toUpperCase()} MODE</small></button>`).join('')}</div><p class="material-description"><strong>${materials[material].tag}</strong><br>${materials[material].rule}</p>${masteryMarkup()}<div class="difficulty-options" role="group" aria-label="난이도">${Object.entries(difficulties).map(([key,d],i)=>`<button data-difficulty="${key}" class="${difficulty===key?'selected':''}" aria-pressed="${difficulty===key}"><span aria-hidden="true">${['❧','▦','♨'][i]}</span> ${d.name}</button>`).join('')}</div><p class="difficulty-note">${difficulties[difficulty].description}</p><div class="duration-panel"><div class="duration-label">플레이 시간 선택</div><div class="menu-tabs" role="group" aria-label="게임 모드">${['sprint','endless','daily'].map(m=>`<button data-mode="${m}" class="${mode===m?'selected':''}" aria-pressed="${mode===m}">${m==='sprint'?'3분':m==='endless'?'∞ 무한':'오늘의 도전'}</button>`).join('')}</div></div><button class="primary start-button" data-screen="start"><span aria-hidden="true">▶</span> ${materials[material].name} 모드 · ${modes[mode]} 시작 <span aria-hidden="true">›</span></button>${lastReplay()?'<button class="secondary" data-screen="last-replay">직전 판 기록에 재도전</button>':''}<div class="lobby-tools"><button class="text-btn" data-screen="tutorial"><span aria-hidden="true">◇</span>조작 연습</button><button class="text-btn" data-screen="help"><span aria-hidden="true">⚙</span>설정·도움말</button></div><div class="menu-best">${mode==='daily'?day()+' · 같은 조각으로 도전':'최고 '+best().toLocaleString()+' · 최고 연쇄 '+(Number(saved.records[bestKey()]?.chain)||0)}</div>`;window.GlassArt?.menu(content);}
...
function finish(reason){if(state==='end')return;state='end';endingReason=reason;resetInput();game.active=null;phase=null;screen.hidden=false;screen.className='screen small';if(mode==='tutorial'){saved.learned=true;save();content.innerHTML='<div class="screen-kicker">FIRST RESONANCE</div><h2>균열이 이어졌어요.</h2><p>완성한 줄에서 충격이 퍼져<br>연결된 유리까지 함께 깨집니다.<br>다음에는 직접 길을 만들어보세요.</p><button class="primary" data-screen="start-sprint">3분 도전 시작</button><button class="secondary" data-screen="tutorial">한 번 더 연습</button>';setStatus('균열이 닿지 않은 조각은 남고, 아래로 내려옵니다.');}
  ...
  if(!gameResultSaved){saved.mastery[material]=after.xp;
   saved.runs=saved.runs.filter(r=>r.mode!==mode||r.seed!==game.seed||(r.material||'glass')!==material||(r.difficulty||'standard')!==difficulty);
   saved.runs.push({mode,material,difficulty,seed:game.seed,attempts,best:sameBest,last:run});saved.runs=saved.runs.slice(-12);
  ...
  content.innerHTML=`<div class="screen-kicker">${game.score>prev?'NEW PERSONAL BEST':'YOUR RESONANCE'}</div><h2>${reason==='time'?'빛나는 3분.':'다음 연결을 향해.'}</h2><div class="result-score">${game.score.toLocaleString()}</div><div class="run-reflection">${reflection}<br>같은 판 최고 ${sameBest.toLocaleString()}점 · ${attempts}회 도전</div><div class="reward-card"><strong>${reward.count}/3 목표 달성 · +${reward.xp} XP</strong><span>${materials[material].name} Lv.${after.level} · ${after.title}${after.level>before.level?' · 승급!':''}</span>${before.level<3&&after.level>=3?'<small>공명 테두리 해금</small>':before.level<5&&after.level>=5?'<small>오로라 테두리 해금</small>':''}</div><details class="goal-results"><summary>이번 판 목표 보기</summary>${goals.map((g,i)=>`<div>${reward.done[i]?'✓':'○'} ${g.label} ${Math.min(game[g.key],g.target)}/${g.target}${g.unit}</div>`).join('')}</details><table class="replay-stats"><caption>직전 동일 조건과 비교</caption><thead><tr><th>기록</th><th>이번</th><th>직전 대비</th></tr></thead><tbody>${rows}</tbody></table><p class="replay-note">조각당 제거량 = 제거한 칸 ÷ 배치한 조각<br>이 모드 최고 연쇄 ${saved.records[bestKey()].chain} · 제거 ${saved.records[bestKey()].extra}</p><p>${reason==='time'?'시간이 끝났어요. 같은 조각으로<br>더 큰 연결을 만들어볼까요?':'조각이 입구까지 차올랐어요.<br>다른 배치로 다시 도전해보세요.'}</p><button class="primary" data-screen="retry">같은 판 최고 기록에 도전</button><button class="secondary" data-screen="new">${mode==='daily'?'오늘의 도전 다시':'새로운 판 시작'}</button><button class="text-btn" data-screen="menu">모드 선택으로</button>`;
...
function beginClear(p){plan=p;chain++;phase='crack';phaseTime=0;state='resolving';resetInput();tone('clear',chain);if(chain>1)callout(chain+' CHAIN','연쇄 배수 ×'+chain);else if(p.extra>0)callout(game.material==='glass'?'RESONANCE':materials[game.material].effect,game.material==='glass'?'균열 +'+p.extra:p.cells.length+'칸 연결');setStatus(game.material==='glass'?`${p.rows.length}줄 완성${p.extra?' · 이어진 유리 '+p.extra+'개 파쇄':''}${chain>1?' · '+chain+'연쇄':''}`:`${materials[game.material].effect} · ${p.cells.length}칸 · ${chain}연쇄`);updateHUD();}
function afterPiece(){strategy=P.strategy(game.board,game.material);if(mode==='tutorial'&&game.lines>0){finish('tutorial');return;}if((mode==='sprint'||mode==='daily')&&remaining<=0){finish('time');return;}if(!game.spawn()){finish('top');return;}state='playing';chain=0;lockTime=0;lockResets=0;fallTime=0;phase=null;plan=null;falls=[];resetInput();renderPreviews();updateHUD();}
...
function cellColors(c){return game.material==='glass'?(colors[c.type]||colors.I):materialColors[game.material][c.paint??0];}
function softCell(c,x,y,size,options){
 const g=options.context||ctx,[light,dark]=cellColors(c),kind=game.material;
 ...
 if(kind==='sand'){
  ...
  g.fillStyle=dark+'55';g.fillRect(px+side*.1,py+side*.1,side*.8,side*.8);
  for(let i=0;i<16;i++){const n=(c.id*37+i*53)%97;g.fillStyle=i%3===0?'#fff0c8':i%3===1?light:dark;const dot=size*(.055+(n%3)*.014);g.beginPath();g.arc(x+size*(.15+(i%4)*.23)+(n%3-1),y+size*(.15+Math.floor(i/4)*.23)+(n%5-2)*.3,dot,0,Math.PI*2);g.fill();}
 }else if(kind==='water'){
