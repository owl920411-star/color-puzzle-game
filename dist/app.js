...
function emitSandLanding(cells){
 ...
 for(let k=0;k<cells.length;k++){
  const c=cells[k],paint=paints[k]?.paint??0,[light,dark]=materialColors.sand[paint]||materialColors.sand[0];
  for(let i=0;i<16&&particles.length<320;i++){
   const life=300+Math.random()*320,side=Math.random()<.5?-1:1;
   particles.push({x:(c.x+.5)*36+(Math.random()-.5)*22,y:(c.y+.72)*36,vx:side*(22+Math.random()*85),vy:-15-Math.random()*70,gravity:360+Math.random()*160,life,total:life,size:.8+Math.random()*1.5,kind:'sand',paint,spark:false,sprite:0,angle:Math.random()*6.28,color:Math.random()<.3?dark:light});
  }
 }
}
function emit(cells){
...
function lock(){if(!game.active)return;const cells=game.active.cells.map(c=>({x:c.x+game.active.x,y:c.y+game.active.y}));if(game.material==='sand')emitSandLanding(cells);impact={x:cells.reduce((n,c)=>n+c.x+.5,0)/cells.length*36,y:Math.max(...cells.map(c=>c.y+1))*36,life:360,total:360};vibrate(8);game.lock();resetInput();chain=0;settleCount=0;if(game.material!=='glass'){beginSettle();return;}const p=E.clearPlan(game.board);if(p)beginClear(p);else afterPiece();}
