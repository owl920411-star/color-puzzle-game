...
let repeat=null,drag=null,gestureFeedbackTime=0,keyHeld=new Set(),soundOn=!!saved.sound,audio=null,renderRatio=1,sandVisuals=[],sandLandingFade=0,sandSurfaceWave=0;
...
function spawnSandVisual(fromX,fromY,toX,toY,cell,power=1){
 if(reduced&&power<2)return;
 const paint=cell?.paint??0,[light,dark]=materialColors.sand[paint]||materialColors.sand[0];
 const count=reduced?6:Math.min(42,12+power*7),cap=reduced?260:850;
 for(let i=0;i<count&&sandVisuals.length<cap;i++){
  const sx=(fromX+.06+Math.random()*.88)*36,sy=(fromY+.12+Math.random()*.76)*36;
  const drift=(toX-fromX)*36,fall=(toY-fromY)*36;
  const tx=sx+drift+(Math.random()-.5)*(8+power*5),ty=sy+fall+6+Math.random()*(8+power*4);
  const life=120+Math.random()*80+power*18;
  sandVisuals.push({x:sx,y:sy,sx,sy,tx,ty,life,total:life,size:.55+Math.random()*(.8+power*.16),color:Math.random()<.3?dark:light,power});
 }
}
function renderSandVisuals(){
 ...
 for(const p of sandVisuals){
  ...
  const arc=4+(p.power||1)*1.7,x=p.sx+(p.tx-p.sx)*e,y=p.sy+(p.ty-p.sy)*e+Math.sin(Math.min(1,t)*Math.PI)*-arc;
...
function emitSandLanding(cells){
 ...
 for(const c of cells){
  const paint=c.paint??c.cell?.paint??0,[light,dark]=materialColors.sand[paint]||materialColors.sand[0];
  for(let i=0;i<count&&sandVisuals.length<700;i++){
   const sx=(c.x+.08+Math.random()*.84)*36,sy=(c.y+.08+Math.random()*.78)*36;
   const spread=(Math.random()-.5)*(reduced?12:28),drop=8+Math.random()*18,life=130+Math.random()*120;
   sandVisuals.push({x:sx,y:sy,sx,sy,tx:sx+spread,ty:sy+drop,life,total:life,size:.55+Math.random()*1.2,color:Math.random()<.24?dark:light});
  }
 }
}
function emit(cells){
...
function lock(){if(!game.active)return;const cells=game.active.cells.map(c=>({x:c.x+game.active.x,y:c.y+game.active.y,paint:c.paint??0,cell:c}));if(game.material==='sand'){emitSandLanding(cells);sandLandingFade=150;sandSurfaceWave=Math.max(sandSurfaceWave,28);}impact={x:cells.reduce((n,c)=>n+c.x+.5,0)/cells.length*36,y:Math.max(...cells.map(c=>c.y+1))*36,life:360,total:360};vibrate(8);game.lock();resetInput();chain=0;settleCount=0;if(game.material!=='glass'){beginSettle();return;}const p=E.clearPlan(game.board);if(p)beginClear(p);else afterPiece();}
...
function renderSandTerrain(){
 ...
 for(let paint=0;paint<game.colorCount;paint++){
  ...
  for(const {x,y} of cells){
   ...
   const wave=!U&&sandSurfaceWave>0?Math.sin(elapsed*.018+x*.92+paint)*Math.min(3.2,sandSurfaceWave*.032):0;
   const top=y*size+(U?0:5+((seed%7)-3)*.7+wave);
