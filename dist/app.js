...
const materials={glass:{name:'유리',tag:'균열을 잇는 설계',rule:'가로줄을 채우면 연결된 흰 균열까지 파쇄됩니다.',effect:'파쇄'},sand:{name:'모래',tag:'산사태를 설계하는 퍼즐',rule:'같은 색 모래를 충분히 모으면 붕괴합니다. 초반 10칸, 이후 12칸부터 SAND BURST! 연쇄 산사태를 노려보세요.',effect:'SAND BURST'},water:{name:'물',tag:'빈틈을 찾아 흐르는 물',rule:'같은 색 물 10칸 이상을 상하좌우로 모으면 배수됩니다.',effect:'배수'},jelly:{name:'젤리',tag:'붙을수록 커지는 탄성',rule:'같은 색 젤리 8칸 이상이 상하좌우로 붙으면 터집니다.',effect:'팝'}};
...
function softCell(c,x,y,size,options){
 ...
 if(kind==='sand'){
  const seed=(c.id*47+(c.paint||0)*83)%101,top=py+side*(.10+(seed%5)*.018);
  g.fillStyle=dark+'b8';g.beginPath();g.moveTo(px,py+side*.28);g.quadraticCurveTo(px+side*.24,top-side*.07,px+side*.48,top);g.quadraticCurveTo(px+side*.74,top+side*.08,px+side,py+side*.23);g.lineTo(px+side,py+side*.93);g.quadraticCurveTo(px+side*.72,py+side,px+side*.48,py+side*.95);g.quadraticCurveTo(px+side*.22,py+side*.99,px,py+side*.9);g.closePath();g.fill();
  ...
  for(let i=0;i<38;i++){const n=(seed+i*37)%113,gx=px+side*(.08+((n*17)%83)/100),gy=top+side*(.08+((n*29+i*11)%72)/100);g.fillStyle=i%7===0?'#fff7d8':i%3===0?dark:light;const dot=Math.max(.65,size*(.012+((n+i)%4)*.005));g.globalAlpha=.55+((n%5)*.08);g.beginPath();g.arc(gx,gy,dot,0,Math.PI*2);g.fill();}
